...
public class Main3 {
	public static void mainfdgf(String[] args) {
		...
		while (true) {
		    ...
		    try {
		        ...
		        if (choice == 1) {
		            ...
		            break;
			        	...
			        	for(KhachHang kh:khachhang) {
			        		if(kh.getMaKhachHang()==mkh) {
			        	        ...
			        	        Boolean stThanhToan;
			        	        HoaDon hd = new HoaDon(maHoaDon, month, year, soDien, kh,stThanhToan=false);
			        	        kh.AddHoaDon(hd);
			        	        Data.KHouputstream(khachhang);
		        	...
		        	int maKhcanoxa= scanner.nextInt();
		        	for(KhachHang kh:khachhang) {
		        		if(kh.getMaKhachHang()==maKhcanoxa) {
		        			khachhang.remove(kh);
		        			System.out.println("Xoa khach hang "+kh.getTenKhachHang()+" thanh cong");
		        			Data.KHouputstream(khachhang);
		        			break;
		        		}
		        		else {
